...
**Changing a card's classroom (step 5) when the class is co-taught:** if the new room is free at that day/time, the change applies immediately for every teacher sharing that class — nothing else to do. If the new room is already taken by someone else at that exact time, the change is held back instead of failing outright: a yellow banner appears at the top of the **Schedule** tab ("A classroom change requested from this schedule, but N teaching block(s) could not move to it automatically because of a room collision"), with a **Resolve pending classrooms** button. See "Resolving a Pending Classroom Conflict" in [Groups](groups.md) for how to resolve it from there — the same screen also opens from a group's own form when a group-wide classroom change hits the same kind of collision.
