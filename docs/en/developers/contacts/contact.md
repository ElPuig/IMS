...
### `_ems_refresh_enrollments_from_template()` — regenerating subject enrollments on a study change
...
```mermaid
flowchart TD
    A["write(values)"] --> B{"'study_id' in values,\nnot env.su,\nno explicit main_group_id?"}
    B -- no --> Z["unchanged: derive level_id only\n(_compute_group_data)"]
    B -- yes --> C["candidates = self.filtered(\neffective contact_type == 'student'\nand study_id actually changes)"]
    C -- empty --> Z
    C -- some --> D["auto_group = first ems.group\nby name for the new study\n(or none)"]
    D --> E["values['main_group_id'] = auto_group.id or False"]
    E --> F["super().write(values)"]
    F --> G["candidates._ems_refresh_enrollments_from_template()"]
```

- **Auto-picked group, not chosen by the user:** the first `ems.group` alphabetically for the new study, with no course filter — a main group's own `name` is built as `study.acronym + course + acronym` (see [`group.md`](group.md)), so ordering by `name` within the study already lands on its lowest course first. Resolved explicitly either way (a found group, or `False`) rather than left as whatever it already was: a direct `write()` bypassing the form's own `_onchange_study_id` (client-side only) would otherwise leave `main_group_id` stuck on a group belonging to the *old* study — the same "incongruence between main_group, level and studies" `_compute_group_data` already guards against for every other path.
...
- **Scoped narrowly, on purpose:**
  - *No explicit `main_group_id` in the same write* — when the caller also picks a group directly (e.g. the secretary sets study **and** group together in one save), that goes through the pre-existing `_migrate_enrollments_on_group_change` cascade above instead, which repoints the *old* (same-study) enrollments into the new group; mixing that with a template-driven refresh for a different study would create rows for subjects belonging to neither curriculum.
  - *`not env.su`* — excludes system flows resolving their own group/subjects on the student's behalf, chiefly `_ems_apply_destination_placement()` (which always writes `main_group_id` and `study_id` together anyway, so it never matches this branch regardless). Same signal `_migrate_enrollments_on_group_change`/`ems.enrollment.default_get()` already use.
...
Covered by `tests/test_contact.py::TestContactStudyChange` (auto-pick, template-driven creation, stale-row removal with/without scored grades, the empty-destination-study case, the explicit-group exclusion, the `env.su` exclusion, the non-student exclusion, and the translated chatter note).
