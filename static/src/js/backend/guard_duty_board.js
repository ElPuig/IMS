...
export class GuardDutyBoard extends Component {
    ...
    async loadBoard() {
        ...
        this.state.board = await this.orm.call(
            ...
            [String(this.state.activeDay), this.state.activeShift, this.activeDate]
        );
    ...
    async onPdfClick() {
        await this.actionService.doAction("ems.action_report_guard_duty_board", {
            additionalContext: {
                ...
                // The printed copy is the screen on paper, absences included - a cuadrante
                // handed out to plan the day's guards is no use without them.
