...
wins, so an already-broken translation survives every deploy. `migrations/18.0.0.23.6/pre-migrate.py`
...
Note for whoever tags the release: the migration script sits in `migrations/18.0.0.23.6/`,
chosen as the next fix-level version above `v18.0.0.23.5`. If #405 (also 18.0.0.23.6) is merged
and tagged before this branch, the folder has to move to the next unreleased version, otherwise
the script never runs on an environment already past that tag. The manifest version is
