...
## Two pre-existing tours broke once "Student Schedule" became a real Print report (found via the full ./test.sh gate):
- The new PDF report (`ems.report_student_schedule`, bound to `res.partner`) means the Students list's control panel now shows a genuine "Print" button alongside "Actions" whenever one or more rows are selected — before this, that list's own bulk-action cluster only ever had "Actions". Two existing tours that select students then open the cog menu (`withdrawal_tour.js`'s generic-Archive-action tour, `portal_access_wizard_tour.js`'s revoke tour) used a bare `.o_cp_action_menus button` trigger, which matched "Print" instead once both buttons existed side by side, so they timed out looking for a menu item ("Archive"/"Portal access...") that was never going to appear under "Print".
- Fixed by narrowing both triggers to `.o_cp_action_menus button:has(.fa-cog)` — already the established disambiguation pattern in this codebase (`attendance_reports_tour.js`). Verified 2/2 clean re-runs. The other tours reaching the same student/applicant list (`student_import_wizard_tour.js`, `student_update_wizard_tour.js`, `applicant_import_wizard_tour.js`) open the cog menu with nothing selected, where Print never renders, so they were never actually at risk - confirmed by reading each, not assumed.
