...
**Trigger 1 and the policy above, resolved 2026-09-06, refined 2026-09-07 — superseded, kept for
history.** 2026-09-06: the developer asked for trigger 1 (test-hang) to follow the exact same
active/inactive/unknown rule as 2 and 3 — *"usa RemoteControl también si parece que se ha quedado
colgado un test. Es lo mismo que los otros."* At that point the fix was: the agent supplements the
hook by also calling `PushNotification` when Remote Control is known active, **in addition to**
the hook's own unconditional file-drop write — deliberately both, since the hook "has no path to
the phone-push mechanism" and touching it seemed out of reach.

**2026-09-07: that "both, always" design turned out to itself be the bug** — with Remote Control
active, the developer was getting the hook's file-drop desktop notification AND the agent's
`PushNotification` phone push for the very same test-launch event ("que antes me has estado
mandando de las dos"). Asked how to resolve it (`AskUserQuestion`), the developer chose to have
the hook itself deduplicate rather than dropping the phone push. The hook *can* be told about
Remote Control's status without ever calling `PushNotification` itself: it now checks a marker
file, `/root/.claude/hooks/.remote-control-active`, and skips its own file-drop write whenever
that file's mtime is fresh (within `RC_MARKER_MAX_AGE_MINUTES`, default 240) — see the hook's own
comments for the exact mechanism. **The agent's job is to keep that marker fresh**: touch it
(`date +%s > /root/.claude/hooks/.remote-control-active`) every time Remote Control is confirmed
active this session (a successful `PushNotification` call, or a session-start check against
memory) — not just once. A stale or missing marker makes the hook fall back to notifying via
file-drop, same safe default as before this change, so forgetting to touch it costs a harmless
duplicate at worst, never silence. With the marker fresh, the agent's own trigger-1
`PushNotification` call (still made exactly as the 2026-09-06 text describes, right before running
a command trigger 1's own classification would flag) is now the *only* notification for that
event — the hook's parallel write is suppressed for as long as the marker stays fresh.
